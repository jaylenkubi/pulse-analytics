 ...
 */
export const getByQueryWebsiteAccesss = (signal?: AbortSignal) => {
  return customInstance<ResponseDtoArray>({
...
export const getGetByQueryWebsiteAccesssQueryKey = () => {
  return [`/website-access/query`] as const;
};
...
>(options?: {
  ...
  const queryKey =
    queryOptions?.queryKey ?? getGetByQueryWebsiteAccesssQueryKey();

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getByQueryWebsiteAccesss>>
  > = ({ signal }) => getByQueryWebsiteAccesss(signal);
...
>(options?: {
  ...
  const queryOptions = getGetByQueryWebsiteAccesssQueryOptions(options);
