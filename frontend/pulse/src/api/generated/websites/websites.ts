 ...
 */
export const getByQueryWebsitess = (
  params?: GetByQueryWebsitessParams,
  signal?: AbortSignal,
) => {
  return customInstance<ResponseDtoArray[]>({
    url: `/websites/query`,
...
export const getGetByQueryWebsitessQueryKey = (
  ...
  return [`/websites/query`, ...(params ? [params] : [])] as const;
};
...
>(
  ...
  const queryKey =
    queryOptions?.queryKey ?? getGetByQueryWebsitessQueryKey(params);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getByQueryWebsitess>>
  > = ({ signal }) => getByQueryWebsitess(params, signal);
...
>(
  ...
  const queryOptions = getGetByQueryWebsitessQueryOptions(params, options);
